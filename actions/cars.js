...
// Ensure price fits DECIMAL(10,2) and is formatted for Prisma
function coerceAndValidatePriceForDb(input) {
  const numericValue = typeof input === "string" ? Number(input) : Number(input ?? NaN);
  if (!Number.isFinite(numericValue)) {
    throw new Error("Invalid price value");
  }
  // Round to 2 decimals
  const rounded = Math.round(numericValue * 100) / 100;
  // For DECIMAL(10,2), absolute value must be < 1e8
  if (Math.abs(rounded) >= 1e8) {
    throw new Error("Price must be less than 100,000,000.00");
  }
  // Provide as string to avoid scientific notation issues
  ...
  try {
    ...
    const car = await db.car.create({
      data: {
        ...
        model: carData.model,
        year: carData.year,
        price: coerceAndValidatePriceForDb(carData.price),
