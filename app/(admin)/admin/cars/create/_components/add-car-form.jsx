...
// Helpers to sanitize numeric fields coming from AI or user input
...
const AddCarForm = () => {
  ...
  useEffect(() => {
    if (processImageResult?.success) {
      ...
      setValue("price", sanitizePriceString(carDetails.price));
      const sanitizedMileage = sanitizeIntegerString(carDetails.mileage);
      setValue("mileage", sanitizedMileage || "0");
  ...
  const onSubmit = async (data) => {
    ...
    const sanitizedPrice = sanitizePriceString(data.price);
    const sanitizedMileage = sanitizeIntegerString(data.mileage) || "0";
    ...
    const carData = {
      ...data,
      year: parseInt(sanitizeIntegerString(data.year)),
      price: parseFloat(sanitizedPrice || "0"),
      mileage: parseInt(sanitizedMileage),
    ...
    // Guard against DECIMAL(10,2) overflow on client too
    if (Math.abs(carData.price) >= 1e8) {
      toast.error("Price must be less than 100,000,000.00");
      return;
    }

    if (!Number.isInteger(carData.mileage) || carData.mileage < 0) {
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                {processImageLoading && (
                  <div className=" text-blue-700 p-4 rounded-md flex items-center border-2 border-blue-500">
                ...
                <div className="bg-amber-50 dark:bg-amber-900 p-4 rounded-md">
